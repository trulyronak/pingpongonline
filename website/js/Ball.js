import { isPrimary, primarySet, roomName, sendBallPosition, socket } from "./socket.js";
...
export class Ball {
  constructor(ctx, board, x, y, z, soundsDiv) {
    ...
    socket.on("recieveballupdate", room => {
  ...
  updatePosition() {
    ...
    if (primarySet && isPrimary) {
      // we control da ball
      sendBallPosition({x: this.x, y: this.y, z: this.z});
    }
  }
